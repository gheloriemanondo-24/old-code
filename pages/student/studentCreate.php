...
$selColl = $_POST['studcollid'] ?? '';
$selDept = $_POST['studcolldeptid'] ?? '';

if ($selColl) {
    $d = $db->prepare("SELECT * FROM departments WHERE deptcollid=? ORDER BY deptfullname");
    $d->execute([$selColl]); $depts = $d->fetchAll();
}
if ($selDept) {
    $p = $db->prepare("SELECT * FROM programs WHERE progcolldeptid=? ORDER BY progfullname");
    $p->execute([$selDept]); $progs = $p->fetchAll();
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['formAction'] ?? '') === 'studCreate') {
    ...
    if ($studid === '')         $errors['studid']        = 'Student ID cannot be empty';
    elseif (!is_numeric($studid)) $errors['studid']      = 'Student ID must be a number';
    ...
    if ($studyear === '' || !is_numeric($studyear)) $errors['studyear'] = 'Year must be a number (1-5)';

    if (empty($errors)) {
        $chk = $db->prepare("SELECT studid FROM students WHERE studid=?");
        $chk->execute([$studid]);
        if ($chk->fetch()) { $errors['studid'] = 'Student ID already exists'; }
        else {
            $db->prepare("INSERT INTO students (studid,studfirstname,studlastname,studmidname,studcollid,studcolldeptid,studprogid,studyear) VALUES(?,?,?,?,?,?,?,?)")
               ->execute([$studid,$studfirstname,$studlastname,$studmidname ?: null,$studcollid,$studcolldeptid,$studprogid,$studyear]);
            header('Location: index.php?section=student&page=studentList&msg=created');
            exit;
        }
    }
}
...
<div class="form-section">
    ...
    <form method="POST">
        <input type="hidden" name="formAction" value="studCreate">
        <div class="form-row">
            <label>Student ID:</label>
            <input type="number" name="studid" value="<?= htmlspecialchars($studid) ?>">
            <span class="error-msg"><?= $errors['studid'] ?? '' ?></span>
        </div>
        <div class="form-row">
        ...
        <div class="form-actions">
            <button type="submit" name="formAction" value="studCreate" class="btn btn-gray">Save New Student Entry</button>
